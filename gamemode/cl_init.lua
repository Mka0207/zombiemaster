	...
	if MySelf:Team() == TEAM_ZOMBIEMASTER then
		if mouseCode == MOUSE_LEFT then
			...
			elseif placingZombie then
				...
				placingZombie = false
			elseif trapEntity then
				local vector = string.Explode(" ", tostring(trapVector.HitPos))
			
				RunConsoleCommand("vst_placetrigger", vector[1], vector[2], vector[3], trapTrigger)

				trapEntity:Remove()
				trapEntity  = nil
				trapTrigger = nil
			elseif rallyEntity then
				local vector = string.Explode(" ", tostring(trapVector.HitPos))
				
				RunConsoleCommand("vst_placerally", vector[1], vector[2], vector[3], trapTrigger)
...
function GM:CreateGhostEntity(trap, rallyID)
	if trap then
		trapEntity = ents.Create("prop_physics")
		trapEntity:SetModel("models/trap.mdl")
		trapEntity:Spawn()
	else
		rallyEntity = ents.Create("prop_physics")
		rallyEntity:SetModel("models/rallypoint.mdl")
		rallyEntity:Spawn()
...
function GM:Think()
	...
	if MySelf:IsZM() and (trapEntity or rallyEntity) then
		trapVector = TraceLongDistance(gui.ScreenToVector(gui.MousePos()))
		
		if trapVector.HitPos then
			if trapEntity then
				trapEntity:SetPos(trapVector.HitPos)
			end
			
			if rallyEntity then
				rallyEntity:SetPos(trapVector.HitPos)
			end
		end
	end
end
