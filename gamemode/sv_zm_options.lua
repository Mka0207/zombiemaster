...
local function ZM_Power_PhysExplode_SV(ply, command, arguments)
	...
	if ply:GetZMPoints() < GetConVar("zm_physexp_cost"):GetInt() then
		ply:PrintMessage(HUD_PRINTTALK, "Error: Not enough resources!\n")
		return
	end
...
local function ZM_Power_SpotCreate_SV(ply, command, arguments)
	...
	local tr_floor = util.TraceHull({start = location + Vector(0, 0, 25), endpos = location - Vector(0, 0, 25), filter = player.GetAll(), mins = Vector(-17, -17, 0), maxs = Vector(17, 17, 4), mask = MASK_NPCSOLID})
	...
	if ply:GetZMPoints() < GetConVar("zm_spotcreate_cost"):GetInt() then
		ply:PrintMessage(HUD_PRINTTALK, "Error: Not enough resources!\n")
		return
	end
...
local function ZM_Command_NPC(ply, command, arguments)
	if ply:IsZM() then
		...
		for _, entity in pairs(entities) do
			if IsValid(entity) and entity:GetNWBool("selected", false) then
